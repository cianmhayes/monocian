...
def build_patch_dataset(train_data_path, validation_data_path, output_path, nclusters = [8, 16, 24, 32, 64, 128]):
    ...
    for img_path in tqdm(list_all_files(validation_data_path)):
        ...
        for p in patches:
            t = torch.unsqueeze(ToTensor()(p), 0)
            t = t.to(device)
            features = pf(t)
            validation_data[image_to_base64(p)] = features.cpu()
    for cm in clustering_models:
        print()
...
def generate_clustering_model_report(validation_set:Dict[str, torch.Tensor], output_path:str, cm:ClusterTrainer, pf:PatchFeaturizer, device:torch.device):
    ...
    for (p, t) in tqdm(validation_set.items()):
        ...
        if all([len(clustered_output[i]) >= max_samples for i in range(len(clustered_output))]) :
            break
    env = Environment(
        loader=FileSystemLoader(os.path.join(os.path.dirname(__file__), "templates")),
        autoescape=select_autoescape())
    template = env.get_template("clustering_examples.html.jinja")
    with open(os.path.join(output_path, f"minibatch_kmeans_k_{cm.n_clusters()}.html"), "w", encoding="utf-8") as report_file:
